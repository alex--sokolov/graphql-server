...
export const login = async (email: string, password: string): Promise<IToken> => {
  ...
  try {
    const data = await sendRequest(path.join(bandsEndpoint, 'login'), Method.POST, body) as string;
...
export const auth = async (firstName:string, lastName:string, email:string, password:string) => {
  ...
  try {
    const data = await sendRequest(path.join(bandsEndpoint, 'register'), Method.POST, body) as string;
    const result = JSON.parse(data);
    return { ...result, id: result._id };
  } catch (error) {
    console.error(error);
